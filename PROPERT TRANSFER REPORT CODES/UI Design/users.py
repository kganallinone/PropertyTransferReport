...
def user_list_window():
    ...
    user_list.geometry("{0}x{1}+250+50".format(935,500))
    ...
    columns = ('id_number', 'lastname', 'firstname', 'email' ,'usertype', 'department')
    
    user_table = ttk.Treeview(frame1, columns=columns, show='headings', height=20)
    user_table.pack(fill=tk.BOTH)
    
    user_table.column('id_number', width=150, anchor='center')
    user_table.column('lastname', width=150, anchor='center')
    user_table.column('firstname', width=150, anchor='center')
    user_table.column('email', width=150, anchor='center')
    user_table.column('usertype', width=150, anchor='center')
    user_table.column('department', width=150, anchor='center')
    ...
    user_table.heading('department', text='Department')
    
    user_db = db.child("user_accounts").order_by_child("ID").get()
    for dt in user_db.each():
        user_table.insert("",'end',values=(dt.val()['ID'],dt.val()['LN'],dt.val()['FN'],dt.val()['EMAIL'],dt.val()['TYPE'],dt.val()['DEPARTMENT']))

...
def requestlist_window():
    ...
    def on_frame_click(value):
        from resources.py_client.client_openrequest import showptr_window
        main.withdraw()
    ...
    def create_report():
        from resources.py_client.client_request import request_window
        main.withdraw()
        request_window()
    ...
    c_report = ctk.CTkButton(info_frame, command=create_report, text = 'Create Report', font= ctk.CTkFont('Arial', size = 15, weight = "bold"),fg_color='#0077B8', text_color='white', corner_radius = 30)
    ...
    for dt in generated_db.each():
        ...
        if db_email == email and db_remarks == 'PENDING':
            if db_type == "Donation":

...
def requestlist_window2():
    ...
    def on_frame_click(value):
        from resources.py_client.client_transaction import showptr_window
        main.withdraw()
    ...
    t = ctk.CTkLabel(info_frame, text='Transactions |', font= ctk.CTkFont('Arial', size = 16, weight='bold'), text_color='black')
    t.pack(side = 'left', padx=(10,0), pady = 10)
    description = ctk.CTkLabel(info_frame, text=' You can generate the approved file.', font= ctk.CTkFont('Arial', size = 16), text_color='black')
    description.pack(side = 'left' ,padx=(3,0), pady = 10)
    #create report

    c_report = ctk.CTkButton(info_frame, command=create_report, text = 'Request List', font= ctk.CTkFont('Arial', size = 15, weight = "bold"),fg_color='#0077B8', text_color='white', corner_radius = 30)
    ...
    for dt in generated_db.each():
        ...
        if db_email == email and (db_remarks == 'APPROVED' or db_remarks == 'REJECTED') :
            if db_type == "Donation":

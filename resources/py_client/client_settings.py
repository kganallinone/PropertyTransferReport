    ...
    def open_edit_item():
        ...
        def show_data(item):
            ...
            try:
                ...
                property_no.delete(0,len(var1))
                property_no.insert(0,var1)
                property_name.delete(0,len(var2))
                property_name.insert(0,var2)
                description.delete(0,len(var3))
        ...
        def search_items(event):
            ...
            if items:
                ...
                for i, item in enumerate(items):
                    button = ctk.CTkButton(suggestions_frame, text=item, width=search_box.winfo_width(), command=lambda i=item: show_data(i))
                    button.pack(pady=1)
        ...
        def create_suggestions_frame():
            ...
            if suggestions_frame is None:
                suggestions_frame = tk.Frame(edit_item)
                suggestions_frame.configure(bd=1, relief=tk.FLAT, bg="white", highlightbackground='white')
                suggestions_frame.configure(width=search_box.winfo_width())
        ...
        def create_suggestion_location_frame():
            ...
            if suggestion_location_frame is None:
                suggestion_location_frame = tk.Frame(edit_item)
        ...
        sub_frame4_3_2 = ctk.CTkFrame(frame1, fg_color='#F4F4F4')
        sub_frame4_3_2.pack(side = 'right')
        
        
        search_box = ctk.CTkEntry(sub_frame4_3_2, placeholder_text='Search item', width=200, fg_color='white')
        search_box.bind("<KeyRelease>", search_items)
        search_box.pack(side = 'right', pady = 0, padx = 5)
        ...
        frame_loc = ctk.CTkFrame(frame3, fg_color='#F4F4F4')
        frame_loc.pack(side = 'right', pady = (0, 10), padx = 50)
        
        location_entry = ctk.CTkEntry(frame_loc, placeholder_text='Search Location', width = 250, fg_color='white')
        location_entry.bind("<KeyRelease>", search_locations)
        location_entry.pack(side = 'right')
        ...
        def search_item():
            ...
            if len(itemno)== 0:
                ...
                for dt in item_db.each():
                    ...
                    description.insert(0,var3)
                    location_entry.delete(0,len(dtloc))
                    location_entry.insert(0,var4)
                    qty_var.set(var5)
        ...
        def save_item():
            ...
            itemloc = location_entry.get()
            specifyloc = location_entry.get()
            ...
            if len(itemno) == 0 or len(itemname) == 0 or len(itemdescrip) == 0 or len(itemloc) == 0 or itemqty == 0:
                ...
                try:
                    ...
                    db.child("items_info").child(itemno).update(item_data)
                    ...
                    db.child("dashboard_info").child("ITEMNO").update({"MAX": itemdb_max,"NAME": itemdb_name,"NO":item_total})
                    ...
                    location_entry.delete(0,len(specifyloc))
                except:
                    property_no.delete(0,len(itemno))
                    property_name.delete(0,len(itemname))
                    description.delete(0,len(itemdescrip))
    ...
    def open_delete_item():
        ...
        def show_data(item):
            ...
            try:
                ...
                property_no.configure(state="normal")  # Set the entry to normal state
                property_no.delete(0,len(var1))
                property_no.insert(0,var1)
                property_no.configure(state="readonly")  # Set the entry back to readonly state
                property_name.configure(state="normal")  # Set the entry to normal state
                property_name.delete(0,len(var2))
                property_name.insert(0,var2)
                property_name.configure(state="readonly") 
        ...
        def create_suggestions_frame():
            global suggestions_frame
            if suggestions_frame is None:
                suggestions_frame = tk.Frame(delete_window)
                suggestions_frame.configure(bd=1, relief=tk.FLAT, bg="white", highlightbackground='white')
                suggestions_frame.configure(width=search_box.winfo_width())
                suggestions_frame.place(in_=sub_frame4_3_2, x=0, rely=1, relwidth=1, y=1)
        ...
        delete_window.title('Property Transfer | Edit Items')
        delete_window.geometry("{0}x{1}+350+50".format(700,525))
        delete_window.resizable(False, False)
        
        qty_var= StringVar(delete_window)
        qty_var.set("1")
        
        #edit frame
        main_frame = ctk.CTkFrame(delete_window, fg_color='white')
        main_frame.pack(fill='both', expand=True)
        
        #header frame
        header_frame = ctk.CTkFrame(main_frame, fg_color='#313131', height=50, width=delete_window.winfo_width(), corner_radius=0)
        header_frame.pack(fill = 'both', side='top')
        ...
        property_no = ctk.CTkEntry(frame3, width=200, fg_color='white', state="readonly" )
        property_no.pack(side = 'left', padx = 50)
        ...
        location_entry = ctk.CTkEntry(frame_loc, placeholder_text='Search Location', width = 250, fg_color='white', state="readonly")
        location_entry.pack(side = 'right')
        ...
        property_name = ctk.CTkEntry(frame5, width=200, state="readonly", fg_color='white')
        property_name.pack(side = 'left', padx =50)
        ...
        description = ctk.CTkEntry(frame7, width=200, state="readonly", fg_color='white')
        description.pack(side = 'left', pady = 10, padx = 50)
        
        delete = ctk.CTkButton(main_frame, command= lambda: delete_items(), text='DELETE', font=ctk.CTkFont('Arial', size = 18, weight='bold'), fg_color='#0077b8')
        ...
        def delete_items():
            ...
            db.child("items_info").child(itemno).remove()
            ...
            description.configure(state="readonly")
            location_entry.configure(state="normal") 
            location_entry.delete(0,len(itemloc))
            location_entry.configure(state="readonly")
            qty_var.set(0)

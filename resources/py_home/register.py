...
def register_window():
    ...
    register.iconphoto(False, open_img)
    ...
    def signup_function():
            ...
            if len(r_uid) == 0 or len(r_email) == 0 or len(r_pw) == 0 or len(r_cpw) == 0:
                if messagebox.askokcancel("Question","Complete your data"):
                    print(0)
                else:
                ...
                if r_pw == r_cpw:
                    try:
                        ...
                        userdb_name = user_db.val()['NAME']
                        userdb_max = user_db.val()['MAX']
                        user_total = user_db.val()['NO']+1
                        db.child("dashboard_info").child("USERNO").set({"MAX": userdb_max,"NAME": userdb_name,"NO":user_total})
                        register.withdraw()
                        login_window()
                        user_id_entry.delete(0,len(r_uid))
                        user_email_entry.delete(0,len(r_email))
    ...
    reg_form_bg_image = ctk.CTkImage(Image.open('resources\images\HOME\_register_image.png'), size=(register.winfo_screenwidth(), register.winfo_screenheight()))
    ...
    def callback():
        try:
            ...
            if messagebox.askok("ERROR", e ):
                print(e)
    register.protocol("WM_DELETE_WINDOW", callback)
    register.mainloop()

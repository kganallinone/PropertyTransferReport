...
def report_window():
        ...
        if len(ptrno) == 0:
            ...
            def copy_and_rename_excel_file(source_file, destination_file):
                ...
                if one == "on":
                    ...
                    '''
                    querty_documents = db.child("generated_files").order_by_child("PTR").equal_to(ptrno).get()
                    if querty_documents:
                        db.child("generated_files").child(ptrno).update(send_prt_info)
                    else:
                        db.child("generated_files").child(ptrno).set(send_prt_info)
                    '''        
    ...
    #generate report
    gen_report = ctk.CTkButton(main, command=generate_report, text = 'Generate Report', font= ctk.CTkFont('Arial', size = 15, weight = "bold"),fg_color='#0077B8', text_color='white', corner_radius = 30)
    ...
    def callback():
        try:
            ...
            file_path = "./resources/user/data/items.xlsx"

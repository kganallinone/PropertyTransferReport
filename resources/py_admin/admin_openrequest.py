...
def showptr_window(getptr):
    ...
    def approval_report(remarks):
        if remarks == 'APPROVED':
            print('APPROVED')
            db.child("generated_files").child(getptr).update({"REMARKS":"APPROVED"})
        else:
            print('REJECTED')
            db.child("generated_files").child(getptr).update({"REMARKS":"REJECTED"})
    ...
    #reject report
    c_reject = ctk.CTkButton(main, command=lambda val='REJECTED': approval_report(val), text = 'Reject', font= ctk.CTkFont('Arial', size = 15, weight = "bold"), fg_color='#bf0e06', hover_color='#ec4c47',  text_color='white', corner_radius = 30)
    c_reject.pack(side = 'right', pady = 5, padx = (0, 20))
    
    #approve report
    c_approve = ctk.CTkButton(main, command=lambda val='APPROVED': approval_report(val), text = 'Approve', font= ctk.CTkFont('Arial', size = 15, weight = "bold"), fg_color='#187bcd', hover_color='#2a9df4', text_color='white', corner_radius = 30)
